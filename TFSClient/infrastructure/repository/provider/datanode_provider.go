...
func (d *dataNodeProvider) GetChunk(context context.Context, name string, chunkId int, replicaid int, socket domain.Socket) ([]byte, error) {
	conn := checkPeerConnection(socket.Ip, socket.Port)
	if conn == nil {
		return nil, fmt.Errorf("error connecting dataNode")
	}
	defer conn.Close()
	client := config.NewDataNodeServiceClient(conn)

	body := &config.ChunkReadRequest{
		ChunkMetadata: &config.ChunkMetadata{
			Name:      name,
			ChunkId:   int32(chunkId),
	...
	response, err := client.ReadChunk(context, body)
	if err != nil {
		return nil, err
	}
	return response.ChunkBytes, nil
}
